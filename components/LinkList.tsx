  ...
  return (
    <div className="grid gap-4">
      {nonYoutubeLinks.map((link) => (
        <div key={link.id} className="p-4 bg-[#1a1d24] rounded-lg shadow border border-[#3a4149]">
          <h2 className="text-xl font-bold mb-2 text-gray-100">{link.title}</h2>
          <a 
            href={link.url} 
            target="_blank" 
          ...
          {link.description && <p className="mt-2 text-gray-300">{link.description}</p>}
