...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      `, []);
      console.log("Dodano brakujące kolumny reset_token i reset_token_expires jeśli nie istniały");
    } catch (alterError) {
      ...
      try {
        ...
        if (!columns || columns.length < 2) {
          ...
          console.log("Dodano brakujące kolumny po sprawdzeniu ich istnienia");
        }
      } catch (schemaError) {
