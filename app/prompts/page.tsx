...
export default function Prompts() {
  ...
  const handleImageClick = (prompt: LinkType) => {
    const imageUrl = prompt.thumbnailData && prompt.thumbnailMimeType
      ? `/api/media/${prompt.id}/thumbnail`
      : prompt.imageData && prompt.imageMimeType
      ? `/api/media/${prompt.id}`
      : null;
    
    if (imageUrl) {
      setSelectedImageUrl(imageUrl);
      setSelectedImageTitle(prompt.title);
        ...
        {prompts.map((prompt) => (
          <div key={prompt.id} className={`${commonStyles.card} flex flex-col h-full bg-[#1a1d24] shadow-lg hover:shadow-xl transition-shadow p-5`}>
            {prompt.thumbnailData && prompt.thumbnailMimeType && !imageErrors[prompt.id] ? (
              <div 
                className="relative aspect-[16/9] w-full mb-5 bg-[#262b36] rounded-lg overflow-hidden cursor-pointer hover:bg-[#2a2f3c] transition-colors"
